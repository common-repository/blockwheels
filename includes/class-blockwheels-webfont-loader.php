...
if ( ! class_exists( 'Blockwheels_WebFont_Loader' ) ) {
	/**
	 * Download webfonts locally.
	 */
	class Blockwheels_WebFont_Loader {
		...
		public function get_local_files_from_css() {
			...
			$stored     = get_site_option( 'blockwheels_downloaded_font_files', array() );
			$change     = false; // If in the end this is true, we need to update the cache option.

			if ( ! defined( 'BLOCKWHEELS_CHMOD_DIR' ) ) {
				define( 'BLOCKWHEELS_CHMOD_DIR', ( 0755 & ~ umask() ) );
			}

			// If the fonts folder don't exist, create it.
			if ( ! file_exists( $this->get_fonts_folder() ) ) {
				$this->get_filesystem()->mkdir( $this->get_fonts_folder(), BLOCKWHEELS_CHMOD_DIR );
			}
			...
			foreach ( $font_files as $font_family => $files ) {
				...
				if ( ! file_exists( $folder_path ) ) {
					$this->get_filesystem()->mkdir( $folder_path, BLOCKWHEELS_CHMOD_DIR );
				}
			...
			if ( $change ) {
				...
				update_site_option( 'blockwheels_downloaded_font_files', $stored );
			}
		...
		public function get_base_path() {
			if ( ! $this->base_path ) {
				$this->base_path = apply_filters( 'blockwheels_get_local_fonts_base_path', $this->get_filesystem()->wp_content_dir() );
			}
		...
		public function get_base_url() {
			if ( ! $this->base_url ) {
				$this->base_url = apply_filters( 'blockwheels_get_local_fonts_base_url', content_url() );
			}
		...
		public function get_subfolder_name() {
			if ( ! $this->subfolder_name ) {
				$this->subfolder_name = apply_filters( 'blockwheels_get_local_fonts_subfolder_name', 'fonts' );
			}
...
if ( ! function_exists( 'blockwheels_get_webfont_styles' ) ) {
	/**
	 ...
	 */
	function blockwheels_get_webfont_styles( $url, $format = 'woff2' ) {
		$font = new Blockwheels_WebFont_Loader( $url );
		$font->set_font_format( $format );
		return $font->get_styles();
	}
}

if ( ! function_exists( 'blockwheels_get_webfont_url' ) ) {
	/**
	 ...
	 * @return string Returns the CSS.
	 */
	function blockwheels_get_webfont_url( $url, $format = 'woff2' ) {
		$font = new Blockwheels_WebFont_Loader( $url );
		$font->set_font_format( $format );
		return $font->get_url();
	}
